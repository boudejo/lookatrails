...
jQuery._uuid_default_method = 'custom';
...
jQuery.uuid = function (p) {
	if (typeof(p) == 'object' && (typeof(p.prefix) == 'string' || typeof(p.method) == 'string')) {
		jQuery._uuid_default_prefix = (typeof p.prefix != 'undefined') ? p.prefix : '';
		jQuery._uuid_default_method = (typeof p.method != 'undefined') ? p.method : '';
	}
	else {
		p = p || jQuery._uuid_default_prefix || '';
		if (jQuery._uuid_default_method == 'custom') {
			return (p + new UUID());
		} else {
