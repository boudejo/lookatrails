...
module Squirrel
  ...
  class Query
    ...
    class Condition
      ...
      def contains_binary? val #:nodoc:
        @operator = :contains_binary
        @operand = val
        @statement = ''
        self
      end
      
      def sql(statement, val) #:nodoc:
        @operator = :sql
        @operand = val
        @statement = statement
        self
      ...
      def to_find_conditions(join_association = {})
        ...
        when :contains_binary   then    [ "LIKE BINARY",    arg_format,        values ]
        when :sql   then    [ statement,    "",  values ]
        else
          case operand
