...
module CalendarHelper
  ...
  def timesheet_calendar(options = {}, &block)
    raise(ArgumentError, "No year given")  unless options.has_key?(:year)
    
    block                        ||= Proc.new {|d| nil}
    
    defaults = {
      :table_id             => 'timesheet_calendar',
      :table_class          => 'timesheet_calendar',
      :year_nav_url         => nil,
      :min_prev_year        => nil,
      :max_next_year        => nil,
      :year_class           => 'year',
    ...
    prev_year, next_year = [options[:year]-1, options[:year]+1]
    prev_year_nav = next_year_nav = ''
    
    cal = %(<table id="#{options[:table_id]}" class="#{options[:table_class]}" border="0" cellspacing="0" cellpadding="0">)
    0.upto(12) do |month|
      if month == 0 then
        prev_year_nav_url = (!options[:year_nav_url].nil?) ? options[:year_nav_url].gsub('ID', prev_year.to_s) : '/?year=#{prev_year}';
        next_year_nav_url = (!options[:year_nav_url].nil?) ? options[:year_nav_url].gsub('ID', next_year.to_s) : '/?year=#{next_year}';
        prev_year_nav = "<a href=\"#{prev_year_nav_url}\" class=\"year_nav prev_year\">&lt;&lt;</a> " if options[:min_prev_year].nil? || options[:min_prev_year] === true || (options[:year] > options[:min_prev_year])
        next_year_nav = " <a href=\"#{next_year_nav_url}\" class=\"year_nav next_year\">&gt;&gt;</a>" if options[:max_prev_year].nil? || options[:max_prev_year] === true || (options[:year] < options[:max_prev_year])
        cal << "<thead><tr><th class=\"#{options[:year_class]}\">#{prev_year_nav}#{options[:year]}#{next_year_nav}</th>"
      else
        month_name = month_names[month][options[:month_abbrev]]
        cal << '<tbody>' if month == 1
        cal << "<tr><td class=\"#{options[:month_class]}\">#{month_name}</td>"
      end
      month_text = (month < 10) ? '0'+month.to_s : month.to_s
      1.upto(31) do |day|
        if month == 0 then
           cal << "<th class=\"#{options[:day_class]}\">#{day}</th>"
           cal << '</tr></thead>' if day == 31
        else
          day_date = get_date(options[:year], month, day)
          if day_date.nil? then
            cal << "<td class=\"#{options[:no_day_class]}\">#{options[:no_day_text]}</td>"
          else
            day_text = (day < 10) ? '0'+day.to_s : day.to_s
            default_date_id = "#{options[:year]}_#{month_text}_#{day_text}"
            date_text, date_attrs = block.call(day_date, default_date_id, weekend?(day_date))
            date_text  ||= day_date.mday
            date_attrs ||= {}
            date_attrs[:class]  ||= options[:day_class]
            date_attrs[:class]  += " #{options[:weekend_class]}" if weekend?(day_date)
            date_attrs[:class]  += " today" if (day_date == Date.today) and options[:show_today]
            date_attrs[:id]     ||= 
            date_attrs[:title]  ||= day_date.to_s
            date_attrs = date_attrs.map {|k, v| %(#{k}="#{v}") }.join(" ")
            cal << "<td #{date_attrs}>#{date_text}</td>"
          end
          cal << '</tr>' if day == 31
        end
      end
      cal << '</tbody>' if month == 12
    end
    cal << "</table>"
  end
