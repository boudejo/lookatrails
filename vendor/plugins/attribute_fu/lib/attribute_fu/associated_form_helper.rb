module AttributeFu
  ...
  module AssociatedFormHelper
    ...
    # <tt>:method_name</tt> - Specify a custom fields_for method
    #
    ...
    def fields_for_associated(associated, *args, &block)
      ...
      method_name = conf[:method_name] || 'fields_for'
      ...
      @template.send(method_name, name, *args.unshift(associated), &block)
    end
    ...
    # * <tt>:readonly</tt> - specify if all fields should be rendered readonly or not
    ...
    def render_associated_form(associated, opts = {})
      ...
      opts.symbolize_keys!
      (opts[:new] - associated.select(&:new_record?).length).times { associated.build } if opts[:new] && !opts[:new].blank?
      ...
      unless associated.empty?
        ...
        name = opts[:name] if opts[:name] && !opts[:name].blank?
 
        associated.map do |element|
          fields_for_associated(element, (opts[:fields_for] || {}).merge(:name => name, :readonly => readonly)) do |f|
            @template.render({:partial => "#{partial}", :locals => {name.to_sym => element, :f => f}.merge(opts[:locals] || {})}.merge(opts[:render] || {}))
          end
        end
      end
    end
