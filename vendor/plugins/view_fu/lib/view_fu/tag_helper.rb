module ViewFu
  module TagHelper
    ...
    def is_new?
      ...
      (action == "new" || action == "create" || @createable) ? true : false
    end
    ...
    # Check if we're on Show actions
    def is_readonly?
      action = params[:action]
      (action == "show" || @readonly) ? true : false
    end
    
    # Returns unified name for actions
    def action_info
      if is_new? then
        return :new
      end
    ...
    def set_action(action)
      if (action.to_s == "new" || action.to_s == "create") then
        @createable = true
      end
      if (action.to_s == "edit" || action.to_s == "update") then
        @editable = true
      end
      if (action.to_s == "show") then
        @readonly = true
      end
    end
