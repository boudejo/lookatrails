      ...
      private
        def css_options_for_tag(name, options)
          ...
          if name.to_s.downcase =~ /^input$/            
            type, css = options[:type], options[:class].to_s ||= ''
            type = 'text' if type == 'password'
            options[:class] = "#{type.to_s} #{css.to_s}".rstrip if css && !css.split(' ').include?(type)
            options[:class] = 'button ' + options[:class].to_s if type == 'submit' || type == 'button' || type == 'reset'
          end
          options
        end
    end
