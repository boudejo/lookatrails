  ...
  module Helpers
    module Internal
      protected
        ...
        #
        def set_flash(action, type = :notice)
          set_normal_flash(action, type)
          set_flash_now(action, type)
        end
  
        # Sets the regular flash (i.e. flash[:notice] = '...')
        #
        def set_normal_flash(action, type = :notice)
          if f = options_for(action).flash
            flash[type]     = f.is_a?(Proc) ? instance_eval(&f) : options_for(action).flash
          end
        end
  
        # Sets the flash.now (i.e. flash.now[:notice] = '...')
        #
        def set_flash_now(action, type = :notice)
          if f = options_for(action).flash_now
            flash.now[type] = f.is_a?(Proc) ? instance_eval(&f) : options_for(action).flash_now
          end
        end
        ...
        def options_for(action)
          ...
          if ResourceController::FAILABLE_ACTIONS.include? action.first
              if action.last == :fails || action.last == :nochanges || action.last == :vfails
                options = options.send(action.last)
              else
                options = options.send(:success)
              end
          end
          #options = options.send(action.last == :fails ? :fails : :success) 
          options
        end
