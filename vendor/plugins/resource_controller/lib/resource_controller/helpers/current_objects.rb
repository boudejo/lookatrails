  ...
  module Helpers
    module CurrentObjects
      protected
        ...
        # Rebuilds the object and loads it (usefull for ajax requests !!)
        def rebuild_object(rebuild_params = false)
          @object = end_of_association_chain.send parent? ? :build : :new, ((rebuild_params) ? object_params : {})
          instance_variable_set "@#{parent_type}", parent_object if parent?
          instance_variable_set "@#{object_name}", @object
        end
        
        # Enables/Disables no changes check (default enabled)
        #
